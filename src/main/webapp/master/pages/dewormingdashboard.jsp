...
<div class="">
	...
	<div class="row ">
		...
		<div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
			<s:form action="dewormingdashboardFinder" theme="simple"
				id="vaccinationfrm">
					...
					<div class="form-inline">
						...
						<div class="form-group" style="width: 15%;">
							<s:select style="width:100%;" list="vaccineMasterList"
								...
								listValue="vacinname" headerKey="" headerValue="Select Medicine"
			...
			<table class="my-table xlstable" style="width: 100%" id='sss'>
				...
				<tbody>
					<s:if test="Medicinelistvet.size!=0">
						<s:iterator value="Medicinelistvet">
							<tr>

								<td><%=(++i)%>/-<s:property value="vaccinationid" /></td>
								 <td hidden="hidden"><input type="hidden" id="clientid" name="clientid" v></td>
								<%if(loginInfo.isSjivh()){ %>
								<td class="text-center">
								<s:if test="Datecolor==1">
							    <input type="text" readonly="readonly" style="background-color: yellow;" id="dueDate<%=i%>" name="dueDate<%=i%>" value="<s:property value="duedate" />" onchange="updateduedate(this.value,this.id,'<s:property value="mastername" />','<s:property value="ClientId" />','1')">
								</s:if>
								<s:else>
								<input type="text" readonly="readonly"  id="dueDate<%=i%>" name="dueDate<%=i%>" value="<s:property value="duedate" />" onchange="updateduedate(this.value,this.id,'<s:property value="mastername" />','<s:property value="ClientId" />','1')">
								</s:else>
								</td>
								<%}else{ %>
								...
								<td class="text-center"><input type="text"  readonly="readonly" style="background-color: gray;" id="followDate<%=i%>" name="followDate<%=i%>"  onchange="updateFollowup('<s:property value="clientId"/>',this.value,'<s:property value="mastername" />','1')"></td>
							    </s:if>
							    <s:elseif test="FollowupId!=0">
							    
							   <td class="text-center" ><input type="text" disabled="disabled" style="background-color: gray;" id="followDate<%=i%>" name="followDate<%=i%>"  onchange="updateFollowup('<s:property value="clientId"/>',this.value,'<s:property value="mastername" />','1')"></td>
							    ...
							    <td class="text-center"><input type="text" readonly="readonly"  id="followDate<%=i%>" name="followDate<%=i%>"  onchange="updateFollowup('<s:property value="clientId"/>',this.value,'<s:property value="mastername" />','1')"></td>
							    
							    </s:else>
							     <%} %>
							<%-- 	<input type="button" class="btn btn-primary" value="Followup" onclick="updateFollowup(<s:property value="clientId"/>)"> --%>
								...
								<td style="text-align: center;">
									<s:if test="givendate!=''">
										<s:if test="consumption_status==0">
											...
											<a href="#" class="btn btn-primary"
												onclick="openMultiConsumePopupdeworm(<s:property value="clientId"/>)">Consume</a> 
										...
										<s:elseif test="consumption_status==1">
											<%-- <a href="#" class="btn btn-primary"
												...
												Charge</a> --%>
											<a href="#" class="btn btn-primary"
												onclick="openMultiAddChargePopupVetMed(<s:property value="clientId"/>)">Add
												Charge</a>
								...
								<td style="text-align: center;"><s:if
										...
										<a href='#'
											onclick="openPopup('newdewormingchartSlotAvailable?clientId=<s:property value='clientId'/>')"><i
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<%} %>
					...
					<button type="button" class="btn btn-primary"
						onclick="createChargeAndUpdateAccountvet('Charge')">Create
						Charge</button>
					<button type="button" class="btn btn-primary"
						onclick="createChargeAndUpdateAccountvet('cash')">Cash Desk</button>
...
<div id="multiconsumemodel" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">
		...
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title">Medicine Consumption</h4>
			</div>
			<div class="modal-body">
			<form action="savemultipleconsumedataveterinaryBookAppointmentAjax" id="multiVaccineForm">
				<div class="col-lg-12 col-md-12 col-xs-12">
...
<div id="multiaddmodel" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">
		...
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title">Medicine Against Add Charge</h4>
			</div>
			<div class="modal-body">
				<div class="col-lg-12 col-md-12 col-xs-12">
					<div class="col-lg-6 col-md-6 col-xs-12">
				...
				<div class="modal-footer" style="padding-top: 30px;">
					<input type="button" class="btn btn-danger" id="addmodelbtn"
						onclick="consumeAddMedicine()" value="Add Charge">
				</div>
			</div>
