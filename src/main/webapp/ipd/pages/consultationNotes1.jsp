...
table:focus {
  ...
  box-shadow: 0 0 8px rgba(52, 152, 219, 0.6); /* Blue glow */
...
table:hover {
  ...
  background-color: #d6e9f7; /* Slightly darker background */
}
...
table:focus {
  ...
  box-shadow: 0 0 8px rgba(243, 156, 18, 0.6); /* Orange glow */
}
...
document.addEventListener("keydown", function(event) {
	  if (event.key === "Tab") {
	    ...
	    // Apply the focused class to any focusable elements (input, button, table)
	    if (activeElement && (activeElement.classList.contains("input-field") || 
	                          activeElement.classList.contains("focusable-button") || 
	                          activeElement.classList.contains("focusable-table"))) {
	      activeElement.classList.add("focused");

	      // Remove the 'focused' class after animation ends
	      setTimeout(function() {
...
if(document.getElementById("onexaminationlok")){
    ...
    if(document.getElementById('remark')){
    	new nicEditor({buttonList : ['fontSize','bold','italic','underline','strikeThrough','subscript','superscript','html','image'],maxHeight : 150}).panelInstance('remark');
    }
    if(document.getElementById('notes_text')){
	    new nicEditor({buttonList : ['fontSize','bold','italic','underline','strikeThrough','subscript','superscript','html','image'],maxHeight : 100}).panelInstance('notes_text');
	   }
 });
...
window.onload = function() {
    ...
    $('.classD').each(function() { 
		if(this.checked==true){
			...
			/* selected1=selected1+","+this.value;
			selectedhist=selectedhist+","+this.value; */
		}
    ...
    $('.classI').each(function() { 
		if(this.checked==true){
			...
			selectedInvst=selectedInvst+","+this.value; 
		}
    ...
    $('.classN').each(function() { 
		if(this.checked==true){
			...
			selectednotes=selectednotes+","+this.value; 
		}
		
    }); 
    
    $('.classO').each(function() { 
		if(this.checked==true){
			
			
			selectedobs=selectedobs+","+this.value; 
		}
    ...
    $('.classCN').each(function() { 
		if(this.checked==true){
			...
			selectecharges=selectecharges+","+this.value; 
		}
	...
	<s:form action="saveconsultationFormCommonnew" theme="simple">
		...
		<div class="row">
			...
			<%if(!loginInfo.isSimpliclinic()){ %>	
			  ...
			  <%if(loginInfo.isSaimed() || loginInfo.getClinicid1().equals("raiprachi") || loginInfo.isLmh()){ %>
			  ...
			  <%if(!loginInfo.getClinicid1().equals("raiprachi")){ %>
				...
				<div class="col-lg-12 col-md-12 col-xs-12 col-sm-12 <%=rmo%>"
					...
					<table class="table">
						...
						<tbody>
							<tr>
								<td><textarea onkeyup="searchChiefcomplains(this.value,1)"
										class="form-control setbackcolor" id="newchief"
										placeholder="type Chief Complians here"
										style="height: 30px !important"></textarea></td>
								<td><input type="button" value="Save as new"
									onclick="saveChiefcomplainfastJSONCF()" class="btn btn-info" /></td>
							</tr>
							<tr>
								<td width="100%;">
									<%
										ArrayList<Master> finalChifcomplain = (ArrayList<Master>) session.getAttribute("finalChifcomplain");
									%>
									
									
										<table id="searchChiefcomplains" class="table"
										style="height: 200px; display: block; overflow: scroll; width: 100%"; >
							         	<%
											int i = 0;
										%>
										<%
											for (Master master : finalChifcomplain) {
										%>
										<tr>
										
										
										
										<td><input type="checkbox" class="classC"
												onclick="removechiefRowCF(this,'<%=master.getId()%>')"
												value="<%=master.getId()%>" checked='checked' /> <input
												type='hidden' value="<%=master.getId()%>"
												name='chiefcomplain[<%=i%>].id' /></td>
											<td><%=master.getChiefcomplains()%></td>
											<td class="hidden"><a
												onclick="removechiefRowCF('<%=master.getId()%>')"><i
			...
			<%if(loginInfo.isSaimed()){ %>
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<table class="table">
						...
						<tbody>
							<tr>
								<td><textarea onkeyup="searchObservation(this.value,17)"
										class="form-control setbackcolor" id="observation"
										placeholder="type Observation here"
										style="height: 30px !important"></textarea></td>
								<td><input type="button" value="Save as new"
									onclick="saveObservationfastJSONCF()" class="btn btn-info" /></td>
							</tr>
							<tr>
								<td width="100%;">
									<%
									...
									%>
										...
										<table id="searchObservation" class="table"
										style="height: 200px; display: block; overflow: scroll; width: 100%"; >
							         	<%
											int k = 0;
										%>
										<%
											for (Master master1 : finalobservation) {
										%>
										<tr>
										
										
										
										<td><input type="checkbox" class="classO"
												onclick="removeObserRowCF(this,'<%=master1.getId()%>')"
												value="<%=master1.getId()%>" checked='checked' /> <input
												type='hidden' value="<%=master1.getId()%>"
												name='observation[<%=k%>].id' /></td>
											<td><%=master1.getNotes()%></td>
											<td class="hidden"><a
												onclick="removeObserRowCF('<%=master1.getId()%>')"><i
													class='fa fa-trash-o'></i></a></td>
			...
			<%if(loginInfo.isSaimed()){ %>
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<table class="table">
						...
						<tbody>
							<tr>
								<td><textarea onkeyup="searchPastHistory(this.value,3)"
										class="form-control setbackcolor" id="pasthistory"
										placeholder="type History here"
										style="height: 30px !important"></textarea></td>
								<td><input type="button" value="Save as new"
									onclick="saveHistoryfastJSONCF()" class="btn btn-info" /></td>
							</tr>
							<tr>
								<td width="100%;">
									<%
										ArrayList<Master> finalhistory = (ArrayList<Master>) session.getAttribute("finalhistory");
									%>
									
									
										<table id="searchHistory" class="table"
										style="height: 200px; display: block; overflow: scroll; width: 100%"; >
							         	<%
											int i = 0;
										%>
										<%
											for (Master master1 : finalhistory) {
										%>
										<tr>
										
										
										
										<td><input type="checkbox" class="classH"
												onclick="removeHistoryRowCF(this,'<%=master1.getHistoryid()%>')"
												value="<%=master1.getHistoryid()%>" checked='checked' /> <input
												type='hidden' value="<%=master1.getHistoryid()%>"
												name='histories[<%=i%>].id' /></td>
											<td><%=master1.getPasthistory()%></td>
											<td class="hidden"><a
												onclick="removeHistoryRowCF('<%=master1.getHistoryid()%>')"><i
													class='fa fa-trash-o'></i></a></td>
				...
				<div class="col-lg-12 col-xs-12 col-md-12 textprime <%=rmo%>">
				  ...
				  <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12"
					...
					<table class="table">
						...
						<tbody>
							<tr>
								<td><textarea onkeyup="searchInvestigation(this.value)"
										class="form-control setbackcolor" id="investigationName"
										placeholder="type Investigation here"
										style="height: 30px !important"></textarea></td>
								<td><input type="button" value="Save as new"
									onclick="saveInvestigationfastJSONCF()" class="btn btn-info" /></td>
							</tr>
							<tr>
								<td width="100%;">
									<%
										ArrayList<Master> finalinvestigation = (ArrayList<Master>) session.getAttribute("finalinvestigation");
									%>
									
									
										<table id="searchInvestigation" class="table"
										style="height: 200px; display: block; overflow: scroll; width: 100%"; >
							         	<%
											int j = 0;
										%>
										<%
											for (Master master1 : finalinvestigation) {
										%>
										<tr>
										
										
										
										<td><input type="checkbox" class="classI"
												onclick="removeInvestigationRowCF(this,'<%=master1.getId()%>')"
												value="<%=master1.getId()%>" checked='checked' /> <input
												type='hidden' value="<%=master1.getId()%>"
												name='investigations[<%=j%>].id' /></td>
											<td><%=master1.getInvestigation_name()%></td>
											<td class="hidden"><a
												onclick="removeInvestigationRowCF('<%=master1.getId()%>')"><i
													class='fa fa-trash-o'></i></a></td>
			...
			<div class="col-lg-12 col-xs-12 col-md-12 textprime <%=rmo%>">
				...
				<div class="col-lg-12 col-md-12 col-xs-12 col-sm-12 <%=rmo%>"
					...
					<table class="table">
						...
						<tbody>
							<tr>
								<td><textarea onkeyup="searchChargesJSONCF(this.value)"
										class="form-control setbackcolor" id="newcharges"
										placeholder="type diagnosis here"
										style="height: 30px !important"></textarea></td>
								 <td><a href="#" data-toggle="modal" data-target="#insercharges"
													class="btn btn-new btn-info" title="New Charges">New
													Charges</a></td> 
							</tr>
					<tr>
								<td width="100%;">
									...
									%>
										...
										<table id="searchCharges" class="table"
										style="height: 200px; display: block; overflow: scroll; width: 100%"; >
							         	<%
											int l = 0;
										%>
										<%
											for (Master master1 : finalTreatment) {
										%>
										<tr>
										
										
										
										<td><input type="checkbox" class="classCN"
												onclick="removeChargesRowCF(this,'<%=master1.getId()%>')"
												value="<%=master1.getId()%>" checked='checked' /> <input
												type='hidden' value="<%=master1.getId()%>"
												name='charges[<%=l%>].id' /></td>
											<td><%=master1.getChargename()%></td>
											<td class="hidden"><a
												onclick="removeChargesRowCF('<%=master1.getId()%>')"><i
													class='fa fa-trash-o'></i></a></td>
										<td class="hidden"><input type="hidden" id="treatcharge" name="treatcharge" value="<%=master1.getCharge()%>"></td>
										<td class="hidden"><input type="hidden" id="chargetype" name="chargetype" value="<%=master1.getChargetype()%>"></td>
										</tr>
										<%
											l++;
										%>
		...
		<%} %>
				...
				<div class="<%=rmo%>">
					...
					<div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
						...
						<div class="form-group" style="margin-top: 10px;">
							<s:textarea cssClass="form-control" rows="6" maxlength=""
								name="discadvnotes" id="discadvnotes" cssStyle="" />
						</div>
					</div>
			...
			<%if(!loginInfo.isSimpliclinic() && !loginInfo.isTotehosp() && !loginInfo.getClinicid1().equals("raiprachi")){ %>
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					<table class="table">
						...
						<tbody>
							<tr>
								<td><textarea onkeyup="searchNotes(this.value,8)"
										class="form-control setbackcolor" id="newnotes"
										placeholder="type Notes here"
										style="height: 30px !important"></textarea></td>
								<td><input type="button" value="Save as new"
									onclick="saveNotesfastJSONCF()" class="btn btn-info" /></td>
							</tr>
							<tr>
								<td width="100%;">
									<%
										ArrayList<Master> finalnotes = (ArrayList<Master>) session.getAttribute("finalnotes");
									%>
									
									
										<table id="searchNotes" class="table"
										style="height: 200px; display: block; overflow: scroll; width: 100%"; >
							         	<%
											int j = 0;
										%>
										<%
											for (Master master1 : finalnotes) {
										%>
										<tr>
										
										
										
										<td><input type="checkbox" class="classN"
												onclick="removeNotesRowCF(this,'<%=master1.getId()%>')"
												value="<%=master1.getId()%>" checked='checked' /> <input
												type='hidden' value="<%=master1.getId()%>"
												name='notes[<%=j%>].id' /></td>
											<td><%=master1.getNotes()%></td>
											<td class="hidden"><a
												onclick="removeNotesRowCF('<%=master1.getId()%>')"><i
													class='fa fa-trash-o'></i></a></td>
					...
					<br>
					<div class="form-group">
									<label for="exampleInputName2"><b>NOTES TEXT</b></label>
								</div>
					<s:textarea cssClass="form-control" rows="9" maxlength=""
								name="notes_text"
								id="notes_text"></s:textarea>
				</div>
				</div>
				<%} %>
			</div>
...
<div class="modal fade modal-draggable" id="insercharges"
						...
						<div class="modal-dialog modal-sm" role="document">
							<div class="modal-content ">
								<div class="modal-header">
									...
									<h3 class="modal-title" id="myModalLabel">New Charges</h3>
								</div>
								...
								<div class="form-group">
												...
												<select class="form-control" onchange="setToothnum(this.value)">
												<option value="0">Select Tooth Number</option>
