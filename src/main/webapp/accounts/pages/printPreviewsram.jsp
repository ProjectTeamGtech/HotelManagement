					...
					<div class="col-lg-6 col-md-6 col-xs-6 col-sm-6 text-left" style="padding-left:0px;padding-right:0px;">
						...
						<div class="form-group" style="margin-bottom: 0px !important;font-size: <%=font%>">
							...
							<s:if test="ipdconsultant!='' || ipdconsultant!=null">
							  ...
							  <%}else {%>
							   <b> <s:property value="ipdconsultant"/> &nbsp; (<s:property value="Owner_qualification"/>)<s:property value="Description"/> </b>
							<%} %>
							</s:if>
							<s:else>
						...
						<div class="table-responsive"> 
							<table class="table"> 
								<thead> 
									...
									<tr> 
										...
										<th class="th3" style="width: 50%"><b>Service Name (Notes) </b><%-- <a href="#" class="hidden-print" onclick="showinvoicechargesByDate('<s:property value="billsummary"/>',<s:property value="invoiceid"/>,<s:property value="totalAmountx"/>,0)"> <i class="fa fa-arrow-down"></i></a> --%></th> 
										
										<th style="text-align: center;width: 5%" class="th5"><b>Qty</b></th>
										<th class="th2"><b>Unit Charge</b></th> 
										<%if(!loginfo.isBalgopal()){ %>
										<%if(notpcsadmin){ %>
										<th class="th2"><b>Code</b></th> 
										<%}else {%>
										<th class="th2"></th> 
										...
										<th style="text-align:right;width: 7%" class="th6"><b>Amount</b></th> 
									</tr> 
								</thead> 
								...
								<%xx++; %>
	                              ...
	                              <%if(!loginfo.isPackage_access()){ %>
										...
										<%} %>
										<%if(!loginfo.isBalgopal()){ %>
										<td style="text-align:right;color: #5cb85c;"> <b><%=Constants.getCurrency(loginfo)%><s:property value="charge"/></b></td>
	                              <%} %>
	                              </tr>
	                              <s:if test="InvoiceName=='IPD'">
								...
								<tr>
										...
										<td class="">
											<s:if test="pkginvissid==0">
											<b>	<%=Constants.getCurrency(loginfo)%><s:property value="unitcharge"/></b>
											</s:if>
											<s:else>
											</s:else>
								...
								<tr>
										...
										<b>
											...
											<%if(!loginfo.isBalgopal()){ %>
										<td><b><s:property value="tpcode"/></b>
										</td>
										<%} %>
										...
										<p><b><s:property value="taxname1"/></b></p>
										</s:if>
										
										<s:if test="taxname2!=''">
										
										<p><b><s:property value="taxname2"/></b></p>
										</s:if>
										
										<s:if test="taxname3!=''">
										
										<p><b><s:property value="taxname3"/></b></p>
										</s:if>
										</td>
										<%} %>
										<%if(loginfo.getIskunal()!=1){ %>
										...
										<td class="text-right">
											<s:if test="pkginvissid==0">
											<b>	<%=Constants.getCurrency(loginfo)%><s:property value="chargeTotal"/></b>
											</s:if>
							...
							<s:if test="pkgAssessmentList.size()>0">
								...
								<tr>
										...
										<td class="text-right hidden">
											<b><%=Constants.getCurrency(loginfo)%><s:property value="charges"/></b>
									...
									<tr class="settopbac">
										...
										<td class="text-right"><b><%=Constants.getCurrency(loginfo)%><%if(notpcsadmin) {%><s:property value="totalAmountx"/><%}else{ %><s:property value="amtWithouttax"/><%} %></b></td>
									</tr>
								</tbody> 
							</table> 
				...
				<div class="row" id='ml2'>
					...
					<div class="col-lg-5 col-md-5 col-xs-5 col-sm-5 text-left" style="padding-left:0px;padding-right:0px;">
						...
						<div class="form-group" style="margin: 0px 0px 4px 0px;">
								<b><span for="inputEmail3" class="control-label">Payment Mode</span><span>: <s:property value="paymode"/> </b>
						...
						<%if(notpcsadmin){ %>
						<div class="form-group" style="margin: 0px 0px 4px 0px;">
							<b><span for="inputEmail3" class="control-label">Prepared By</span><span>:
							<%if(loginfo.getIskunal()==1) {%> 
							<s:property value="userid"/></span></b>
							<%}else{%>
							<b><s:property value="preparedby"/></span></b>
							<%} %>
						</div>
						<%} %>
